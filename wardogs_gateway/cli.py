...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "serve":
            ...
            # Factory vermeiden, damit der frei wählbare Config-Pfad sicher ankommt.
            from .app import create_app

            uvicorn.run(create_app(args.config), host=host, port=port, reload=args.reload)
            return 0
        ...
        if args.command == "db-init":
            ...
            for record in records:
                ...
                print(
                    f"{record.public_id}  {record.name!r}  role={record.role}  mode={mode}  "
                    f"active={record.active}  expires={record.expires_at or '-'}  last={record.last_used_at or '-'}"
                )
            ...
            print("OK: deaktiviert")
        elif args.command == "key-enable":
            store.set_active(args.id, True)
            print("OK: aktiviert")
        elif args.command == "key-delete":
            store.delete(args.id)
            print("OK: gelöscht")
        elif args.command == "key-rotate":
            record, raw_key = store.rotate(args.id)
            _print_record(record)
            print("\nNEUER API KEY (wird nicht erneut angezeigt):")
            print(raw_key)
        elif args.command == "key-set-role":
            store.set_role(args.id, args.role)
            print("OK: Rolle geändert")
        elif args.command == "key-set-permissions":
            store.set_permissions(args.id, None if args.inherit_role else args.allow)
            print("OK: Berechtigungen geändert")
        return 0
    except (ValueError, FileNotFoundError, RuntimeError) as exc:
        print(f"FEHLER: {exc}", file=sys.stderr)
        return 2
    except KeyError as exc:
        print(f"FEHLER: API-Key-ID nicht gefunden: {exc.args[0]}", file=sys.stderr)
        return 3
